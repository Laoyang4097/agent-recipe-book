...
/* C 级放行阈值（R-10 标定初值，2026-09-24 回测）
   相对归一化：某结果 score / 该查询 Top1 score ≥ 此值，视为「高相关」可放行。
   标定（_calibrate_match_threshold.js，8 正例查询）三档 0.8/0.85/0.9 真噪声率均 0%，
   故 0.8 作初值；赛后 M5 真实用户数据回测可微调。改此数 = 改 C 级放行松紧。

   ⚠️ 治理铁律（2026-09-24 拍板）：阈值**只由我们/自托管者控制**，绝不暴露为 MCP 工具参数
   —— 它是质量/安全闸门，开放原始浮点（如 0.1）会让 C 级垃圾全进主检索，L3 设计形同虚设。
   自托管者想调自己库的松紧，设环境变量 `RECIPE_THRESHOLD`（如 0.85）即可，不必改源码。
   浏览器端 `process` 不存在，故用 typeof 守卫。 */
const ENV_THRESHOLD =
  (typeof process !== "undefined" && process.env && process.env.RECIPE_THRESHOLD)
    ? Number(process.env.RECIPE_THRESHOLD)
    : NaN;
export const C_LEVEL_MATCH_THRESHOLD =
  Number.isFinite(ENV_THRESHOLD) && ENV_THRESHOLD >= 0 && ENV_THRESHOLD <= 1
    ? ENV_THRESHOLD
    : 0.8;
...
/* 单字符不足以定位（37 条库时期实测输入 "a" 命中 35/37 条，见 PRD §6.3 缺陷 C）。
   注意：这条只对「慢库」成立。库涨到 42 条后重新跑过，结论方向不变，
   但数字会随库规模漂移 —— 想复现请跑 tests/search-baseline.js 而不是照抄这里的数。 */
...
/* ---------- L3 置信权重（R-10，2026-09-24 落地） ----------
   A=已验证(×1.0) / B=完整未验证(×0.85) / C=缓刑隔离(×0)。
   缺省字段按 A 处理（42 条冷启动全标 A，兼容旧数据）。 */
export const CONFIDENCE_WEIGHT = { A: 1.0, B: 0.85, C: 0 };
export function confidenceOf(recipe) {
  const c = recipe && recipe.confidence;
  return c === "A" || c === "B" || c === "C" ? c : "A";
}
function effectiveScore(raw, conf) {
  return raw * (CONFIDENCE_WEIGHT[conf] ?? 1.0);
}

/* ---------- 主入口 ----------
   searchRecipes(recipes, query, opts) -> {
     query, terms, synTags, matched, results: [{ score, recipe, matchPct, confidence, suspect }],
     returned, has_more, quarantined_count, hint
   }
   ...
   - 结果按有效分（含 L3 权重）降序；score 相同时保持入参顺序（Array.sort 稳定）
   - 翻页（R-10 讨论落地，2026-09-24）：opts.offset 从第几条切（默认 0）；
     returned=本页条数；has_more=后面还有无更多（agent 见 true 应翻页/调大 limit 再查，
     才可下"库里没有"结论，避免卡在第 6 条）。
   - L3 隔离（R-10）：
       · C 级默认不进主检索（matched / 主结果均不含 C）；
       · 但命中结果中 matchPct 最高者若为 C 且 ≥ C_LEVEL_MATCH_THRESHOLD，破例露头并标 suspect（§6.6.1）；
       · opts.include_quarantine=true 时，主结果包含全部 C 级（均标 suspect），供审计/复核（AC-3）。 */
export function searchRecipes(recipes, query, options = {}) {
  ...
  /* 翻页边界（offset 仅向下取整、负数/越界安全夹紧） */
  const pageStart = offset > 0 && Number.isFinite(offset) ? Math.floor(offset) : 0;
  const pageEnd = limit === Infinity ? undefined : pageStart + limit;
  ...
  // 候选池里 C 级计数（审计用，AC-3）
  const quarantined_count = pool.filter((r) => confidenceOf(r) === "C").length;
  ...
  if (!terms.length) {
    if (rawQuery === "") {
      const chosen = includeQuarantine ? pool : pool.filter((r) => confidenceOf(r) !== "C");
      const page = chosen.slice(pageStart, pageEnd);
      return {
        query: rawQuery, terms: [], synTags: [], matched: chosen.length,
        quarantined_count,
        results: page.map((recipe) => ({
          score: 0, recipe, matchPct: 0,
          confidence: confidenceOf(recipe),
          suspect: confidenceOf(recipe) === "C",
        })),
        returned: page.length,
        has_more: chosen.length > pageStart + page.length,
        hint: null,
      };
    }
    return {
  ...
  const scored = [];
  for (const r of pool) {
    const raw = scoreRecipe(r, terms, synTags);
    if (raw <= 0) continue;
    scored.push({ raw, recipe: r, conf: confidenceOf(r) });
  }
  scored.sort((a, b) => effectiveScore(b.raw, b.conf) - effectiveScore(a.raw, a.conf));
  const topRaw = scored.length ? scored[0].raw : 0;

  // 相对匹配度%（§6.6.1）：该结果 raw / 本查询 Top1 raw
  for (const s of scored) {
    s.matchPct = topRaw > 0 ? Math.round((s.raw / topRaw) * 100) : 0;
  }

  // C 级例外放行（§6.6.1）：命中中 matchPct 最高者若为 C 且 ≥ 阈值，破例露头
  const topByPct = scored.reduce(
    (best, s) => (s.matchPct > (best ? best.matchPct : -1) ? s : best),
    null,
  );
  const releasedC =
    topByPct && topByPct.conf === "C" && topByPct.matchPct >= C_LEVEL_MATCH_THRESHOLD
      ? topByPct
      : null;

  // 主结果 = 非 C，或破例放行之 C；includeQuarantine 时含全部 C
  const main = scored.filter((s) => s.conf !== "C" || s === releasedC);
  const chosen = includeQuarantine ? scored : main;
  ...
  return {
    ...
    returned: page.length,
    has_more: chosen.length > pageStart + page.length,
    hint: chosen.length ? null : HINT_NO_MATCH,
  };
}
