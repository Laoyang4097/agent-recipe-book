...
solution: ① 用「内容比对」而非「引用计数」判断同步：逐文件对比本地内容与远端 API 返回内容，或对比 tree SHA；② diff 基准改用本地 HEAD~1
  等本地存在的提交；③ 网络恢复后在正常网络执行一次 git fetch && git reset --hard origin/main 把指针对齐。
result: 内容层面确认本地与远端零差异（本地有远端无 = 无、远端有本地无 = 无）；推送脚本改为运行时动态获取远端 SHA 作父提交，不再依赖本地引用。
retrospective: git 的本地引用是缓存而非真相。绕过 git 推送时本地状态会失真 —— 判断同步与否要看内容，不要看计数器。
harness: WorkBuddy
hardware:
  os: Windows 10 22H2
  cpu: i5-10210U
